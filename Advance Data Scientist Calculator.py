# ==========================================
# ADVANCED DATA SCIENCE & BUSINESS ANALYTICS CALCULATOR
# Single-file Tkinter app (Full Integrated Version)
...
# Try elegant script fonts with fallback
...
class HistoryPanel:
    ...
    def _select(self, _event=None):
        ...
        if not sel:
            return
        # convert displayed index (newest first) back to CALC_HISTORY index
...
class AdvancedCalculatorApp:
    def __init__(self, root):
        ...
        # ----- Root layout: Notebook (row 0) + Footer (row 1) -----
        root.grid_rowconfigure(0, weight=1)  # Notebook grows
        root.grid_rowconfigure(1, weight=0)   # footer fixed height
        root.grid_columnconfigure(0, weight=1)

        self.notebook = ttk.Notebook(root)
        self.notebook.grid(row=0, column=0, sticky="nsew")

        # ----- Global footer signature (visible on all tabs) -----
        footer = tk.Frame(root, bg=BG)
        footer.grid(row=1, column=0, sticky="ew")
        footer.grid_columnconfigure(0, weight=1)

        # Try signature fonts with fallback
        ...
        sig = tk.Label(
            ...
            text=SIGNATURE_TEXT,
            bg=BG,
            fg="#e9e13d",          # visible on dark background
            font=signature_font
        )
        ...
        # Auto-open Basic tab + focus input
    ...
    def _add_tab(self, name, TabClass):
        ...
        # Single container for tab UI (no scroll)
...
class BasicCalculatorTab:
    def __init__(self, parent):
        ...
        # Readonly display (prevents double-typing bugs)
        ...
        # Keyboard works without clicking:
    ...
    def _safe_eval(self, expr: str):
        # Allow only arithmetic; no names.
...
class ScientificCalculatorTab:
    def __init__(self, parent):
        ...
        # Keyboard focus without clicking
        self.display.focus_set()
        self.display.bind("<KeyPress>", self.handle_key)
        self.display.bind("<<Paste>>", self.handle_paste)
    ...
    def _safe_eval(self, expr: str):
        ...
        # Allow only digits, operators, dot, parentheses, percent, constants pi/e, and spaces
    ...
    def on_press(self, value):
        ...
        # unary functions act instantly (real calculator)
...
class MatrixTab:
    def __init__(self, parent):
        ...
        # Operations row 2 (B specific + Reset)
    ...
    def reset_matrices(self):
        ...
        # Destroy all widgets inside matrix frames (including old Entry boxes)
        ...
        # Clear output safely (it may be disabled)
        try:
    ...
    def show_output(self, obj):
        ...
        # Nicer formatting for matrices/vectors
        try:
    ...
    def solve_ax_b(self):
        ...
        try:
            ...
            # allow B as vector (n,1) or matrix (n,k)
            if B.ndim != 2:
...
class MLTab:
    def __init__(self, parent):
        ...
        self.model = None
        self._last = {}
        self._dummy_columns = None  # columns after get_dummies
        self._x_cols = []           # original feature cols (before encoding)
        self._y_col = None

        tk.Label(parent, text="Machine Learning Tools", bg=BG, fg=FG, font=FONT_TITLE).pack(pady=10)

        # =========================
        # Layout (matches your mockup)
        # Top-left  : Steps + Load buttons
        # Top-right : File data preview
        # Bottom-left: Inputs/Selections
...
Tip: For prediction, enter values in the same order as selected Features."""
        ...
        self.preview.configure(
            ...
            fg=FG,
            insertbackground=FG,
            font=("Consolas", 11),   # <-- smaller as you asked
            wrap="none"
        )

        # --- Inputs panel (bottom-left)
    ...
    # --------------------------
    # Styling for color-coded metrics
    # --------------------------
    ...
    def train_model(self):
        ...
        if alg == "Linear Regression":
            ...
            self._insert_line("-" * 50, "neutral")
            # Pretty print confusion matrix with spacing
            try:
...
class FinanceTab:
    ...
    def calc_cagr(self):
        P = self._get_float("principal")
        FV = self._get_float("rate")  # reuse rate input as FV for CAGR convenience
        t = self._get_float("time")
    ...
    def break_even(self):
        # convenience: principal=fixed cost, rate=price, time=variable cost
        try:
    ...
    def what_if(self):
        # convenience: rate=price, principal=volume, time=unit cost
        try:
            price = float(self.entries["rate"].get())
...
if __name__ == "__main__":
    ...
    root.state("zoomed")   # Windows fullscreen (maximized)
