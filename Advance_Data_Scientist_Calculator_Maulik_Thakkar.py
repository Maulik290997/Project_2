...
class BasicCalculatorTab:
    ...
    def calculate(self):
        ...
        try:
            ...
            if "%" in expr_eval:
                expr_eval = self._handle_percentage(expr_eval)

            result = self._safe_eval(expr_eval)
    ...
    def _handle_percentage(self, expr):
        ...
        return expr.replace(match.group(0), f"{base}{op}{perc_value}")
...
class ScientificCalculatorTab:
    def __init__(self, parent):
        ...
        left = tk.Frame(main, bg=BG)
        left.pack(side="left", fill="both", expand=True)
        self.parent.bind_all("<Button-1>", lambda e: self._refocus_display())
    ...
    def _apply_unary(self, name, func):
        ...
        try:
            ...
            self.history.refresh()
            self.just_evaluated = True   # ✅ ADD THIS
        except Exception:
            self._set_value("Error")
            self.just_evaluated = True   # (optional, but nice)
        self.display.focus_set()
        self._refocus_display()

    def _refocus_display(self):
        try:
            # run AFTER the button click event finishes
            self.parent.after_idle(lambda: self.display.focus_set())
        except Exception:
            pass
    ...
    def _safe_eval(self, expr: str):
        ...
        cleaned = self._preprocess_percent(cleaned)
        cleaned = cleaned.replace("^", "**")

        # ✅ allow letters + underscore for function names (sin, asin, sqrt, etc.)
        if re.search(r"[^0-9A-Za-z_\+\-\*\/\(\)\.\,\s]", cleaned):
            raise ValueError("Invalid characters")

        return eval(cleaned, {"__builtins__": {}}, self._env())


    def _preprocess_percent(self, expr: str) -> str:
        """
        Calculator-style percent handling:
        - a+b%  => a + (a*b/100)
        - a-b%  => a - (a*b/100)
        - standalone x% => x/100
        Works even if 'a' is an expression like (10+5) or sin(30).
        """
        s = expr.strip()

        
        percent_pat = re.compile(r"^(.*?)([+\-])\s*(\d+(?:\.\d+)?)%\s*$")

        m = percent_pat.match(s)
        if m:
            base_expr = m.group(1).strip()
            op = m.group(2)
            perc = m.group(3)

            return f"({base_expr}){op}(({base_expr})*({perc}/100))"
...
class DataTab:
    def __init__(self, parent):
        ...
        # view controls
        self.preview_rows_var = tk.StringVar(value="50")   # user can change
        ...
        view_frame.pack(pady=(8, 2), fill="x")
        tk.Label(view_frame, text="Rows to view:", bg=BG, fg=FG).pack(side="left", padx=(10, 6))
        self.rows_entry = tk.Entry(view_frame, width=8, textvariable=self.preview_rows_var)
        self.rows_entry.pack(side="left")

        tk.Button(view_frame, text="Show", bg=BTN, fg=FG, command=self.refresh_preview).pack(side="left", padx=(10, 6))

        tk.Checkbutton(
            view_frame, text="Full data", variable=self.show_all_var,
            onvalue=True, offvalue=False,
            bg=BG, fg=FG, selectcolor=BG, activebackground=BG, activeforeground=FG,
            command=self.refresh_preview
        ).pack(side="left", padx=(6, 6))

        tk.Button(view_frame, text="Reset", bg=BTN, fg=FG, command=self.reset_data_tab).pack(side="right", padx=(6, 10))

        # Scrolled preview area (shows full data with scrollbars)
        self.preview_frame, self.text = make_scrolled_text(parent, height=18, width=140)
        self.preview_frame.configure(bg=BG)
        self.text.configure(bg="#000000", fg=FG, insertbackground=FG, font=("Consolas", 11), wrap="none")

        col_frame = tk.Frame(parent, bg=BG)
        col_frame.pack(pady=5)
        tk.Label(col_frame, text="Column:", bg=BG, fg=FG).pack(side="left")
        self.column_var = tk.StringVar()
        self.column_menu = ttk.Combobox(col_frame, textvariable=self.column_var, width=30, state="readonly")
        ...
        filter_frame.pack(pady=8, fill="x")
        tk.Label(filter_frame, text="Filter (pandas query):", bg=BG, fg=FG).pack(side="left", padx=(10, 6))
        self.filter_entry = tk.Entry(filter_frame, width=55)
        ...
        hint = tk.Label(
            parent,
            text="Hint: COLUMN_NAME > 5000   (Example query)",
            bg=BG, fg="#cfcfcf", font=FONT_SMALL
        )
        hint.pack(pady=(0, 6), anchor="w", padx=12)

    def _get_preview_rows(self):
        """Return int rows to view, or None for full."""
        if self.show_all_var.get():
            return None
        raw = (self.preview_rows_var.get() or "").strip().upper()
        if raw in ("ALL", "FULL", "*"):
            return None
        try:
            n = int(float(raw))
    ...
    def reset_data_tab(self):
        """Reset view + filter controls (keeps loaded dataframe)."""
        try:
    ...
    def refresh_preview(self):
        """Re-render preview with current row/full settings."""
        if self.df is None:
            self.update_preview("No data loaded.")
            return
    ...
    def update_columns(self):
        if self.df is None:
            return
        self.column_menu["values"] = list(self.df.columns.astype(str))
        if len(self.df.columns) > 0:
            self.column_var.set(str(self.df.columns[0]))
    ...
    def update_preview(self, df_or_series):
        ...
        try:
            ...
            if isinstance(df_or_series, pd.DataFrame):
                n = self._get_preview_rows()
                show_df = df_or_series if n is None else df_or_series.head(n)
                self.text.insert(tk.END, show_df.to_string())
                if n is None:
                    self.text.insert(tk.END, f"\n\n--- Showing FULL dataset ({len(df_or_series)} rows) ---")
                else:
                    self.text.insert(tk.END, f"\n\n--- Showing first {min(n, len(df_or_series))} of {len(df_or_series)} rows ---")
                return

            # Series / numpy / other objects with to_string()
            if hasattr(df_or_series, "to_string"):
                self.text.insert(tk.END, df_or_series.to_string())
            else:
...
class ForecastTab:
    def __init__(self, parent):
        ...
        self.col_var = ttk.Combobox(col_frame, width=30, state="readonly")
        ...
        self.method = ttk.Combobox(
            method_frame, width=28, state="readonly",
        ...
        tk.Button(method_frame, text="Run Forecast", bg=ACCENT, fg="black", command=self.run_forecast)            .pack(side="left", padx=12)

        # --- Preview controls (same behavior as Data Tools) ---
        ...
        tk.Label(view_frame, text="Rows to view:", bg=BG, fg=FG).pack(side="left", padx=(10, 6))
        tk.Entry(view_frame, width=8, textvariable=self.preview_rows_var).pack(side="left")
        ...
        tk.Button(view_frame, text="Reset", bg=BTN, fg=FG, command=self.reset_forecasting)            .pack(side="right", padx=(6, 10))

        # Scrollable preview/output area
        self.preview_frame, self.output = make_scrolled_text(parent, height=18, width=140)
        self.output.configure(bg="#000000", fg=FG, insertbackground=FG, font=("Consolas", 11), wrap="none")
    ...
    def _write_ready(self):
        self.output.delete("1.0", tk.END)
        self.output.insert(tk.END, "Load CSV/Excel to preview data.\nThen select a numeric column and run forecast.\n")
    ...
    def refresh_preview(self):
        ...
        self.output.delete("1.0", tk.END)
        n = self._get_preview_rows()
        show_df = self.df if n is None else self.df.head(n)
        self.output.insert(tk.END, show_df.to_string())
        if n is None:
            self.output.insert(tk.END, f"\n\n--- Showing FULL dataset ({len(self.df)} rows) ---")
        else:
            self.output.insert(tk.END, f"\n\n--- Showing first {min(n, len(self.df))} of {len(self.df)} rows ---")

    def reset_forecasting(self):
        self.preview_rows_var.set("50")
        self.show_all_var.set(False)
    ...
    def _after_load(self):
        if self.df is None:
            return
        # suggest numeric columns first (but allow all if needed)
        cols = list(self.df.columns.astype(str))
        num_cols = []
        for c in cols:
            ...
            if s.notna().sum() > 0:
                num_cols.append(c)
        self.col_var["values"] = num_cols if num_cols else cols
        if num_cols:
            self.col_var.set(num_cols[0])
        elif cols:
            self.col_var.set(cols[0])
        self.reset_forecasting()

    # ---------- Forecast logic ----------
    def run_forecast(self):
        if self.df is None:
            return messagebox.showerror("Error", "Load a dataset first.")
        col = (self.col_var.get() or "").strip()
        if not col:
            return messagebox.showerror("Error", "Select a column.")

        # IMPORTANT: Forecast uses FULL dataset (not the preview)
        series = pd.to_numeric(self.df[col], errors="coerce").dropna().reset_index(drop=True)

        if len(series) < 10:
            return messagebox.showerror("Error", "Need at least 10 numeric points in selected column.")
    ...
    def _show_plot(self, series, line, title, future_x=None, future_values=None):
        # Bigger default; will resize responsively
        ...
        # Responsive resize (so fullscreen works)
        resize_job = {"id": None}
        ...
        def on_resize(event=None):
            ...
            if resize_job["id"] is not None:
                try:
                    win.after_cancel(resize_job["id"])
                except Exception:
                    pass

            def _do():
                try:
                    w = max(win.winfo_width(), 300)
                    h = max(win.winfo_height(), 300)
            ...
            resize_job["id"] = win.after(120, _do)
    ...
    def simple_moving_average(self, series, window):
        ...
        forecast = float(sma.dropna().iloc[-1])
...
class PlotTab:
    def __init__(self, parent):
        ...
        tk.Button(view_frame, text="Show", bg=BTN, fg=FG, command=self.refresh_preview)\
        ...
        tk.Button(view_frame, text="Reset", bg=BTN, fg=FG, command=self.reset_plot_tab)\
            .pack(side="right", padx=(6, 10))

        # scrollable preview
        self.preview_frame, self.output = make_scrolled_text(parent, height=14, width=140)
        self.output.configure(font=("Consolas", 11), wrap="none")

        col_frame = tk.Frame(parent, bg=BG)
        col_frame.pack(pady=5)
        tk.Label(col_frame, text="Column X:", bg=BG, fg=FG).pack(side="left")
        self.col_x = ttk.Combobox(col_frame, width=20, state="readonly")
        self.col_x.pack(side="left", padx=8)
        tk.Label(col_frame, text="Column Y:", bg=BG, fg=FG).pack(side="left")
        self.col_y = ttk.Combobox(col_frame, width=20, state="readonly")
        self.col_y.pack(side="left", padx=8)
    ...
    def refresh_preview(self):
        ...
        if n is None:
            self.output.insert(tk.END, f"- Showing FULL dataset ({len(self.df)} rows) ---")
        else:
            self.output.insert(tk.END, f"\n- Showing first {min(n, len(self.df))} of {len(self.df)} rows ---")
    ...
    def _show_fig(self, fig, title):
        """Show a resizable Matplotlib figure inside a Tk window."""
        try:
            fig.tight_layout()
        except Exception:
            pass
        ...
        # Make figure responsive on window resize
    ...
    def scatter_plot(self):
        ...
        if not x or not y:
            return messagebox.showerror("Error", "Select Column X and Column Y.")
        try:
            x_s = pd.to_numeric(self.df[x], errors="coerce")
            y_s = pd.to_numeric(self.df[y], errors="coerce")
            d = pd.DataFrame({"x": x_s, "y": y_s}).dropna()
            if len(d) == 0:
                return messagebox.showerror("Error", "Selected columns have no numeric overlap.")
        except Exception:
            return messagebox.showerror("Error", "Scatter needs numeric X and Y.")

        fig = plt.figure(figsize=(9, 5.5))
        ax = fig.add_subplot(111)
        ax.scatter(d["x"].values, d["y"].values)
        ax.set_xlabel(x)
        ax.set_ylabel(y)
    ...
    def line_plot(self):
        ...
        if not x or not y:
            return messagebox.showerror("Error", "Select Column X and Column Y.")
        try:
            # Allow X to be anything sortable (date/category), Y must be numeric
            y_s = pd.to_numeric(self.df[y], errors="coerce")
            d = pd.DataFrame({"x": self.df[x], "y": y_s}).dropna()
            if len(d) == 0:
                return messagebox.showerror("Error", "Column Y has no numeric data.")
        except Exception:
            return messagebox.showerror("Error", "Line plot failed (check data).")
    ...
    def histogram(self):
        ...
        x = self.col_x.get()
        if not x:
            return messagebox.showerror("Error", "Select Column X.")
        s = pd.to_numeric(self.df[x], errors="coerce").dropna()
        if len(s) == 0:
            return messagebox.showerror("Error", "Selected column has no numeric data.")

        fig = plt.figure(figsize=(9, 5.5))
        ax = fig.add_subplot(111)
        ax.hist(s.values, bins=25)
        ax.set_xlabel(x)
    ...
    def bar_chart(self):
        ...
        if not x or not y:
            return messagebox.showerror("Error", "Select Column X and Column Y.")
        y_s = pd.to_numeric(self.df[y], errors="coerce")
        d = pd.DataFrame({"x": self.df[x].astype(str), "y": y_s}).dropna()
        if len(d) == 0:
            return messagebox.showerror("Error", "Column Y has no numeric data.")

        # Aggregate if X has many duplicates
        agg = d.groupby("x", as_index=False)["y"].sum()

        # If too many categories, show top 30 by value to avoid unreadable chart
        if len(agg) > 30:
            agg = agg.sort_values("y", ascending=False).head(30)

        fig = plt.figure(figsize=(10, 6))
        ax = fig.add_subplot(111)
        ax.bar(agg["x"].values, agg["y"].values)
        ax.set_xlabel(x)
        ax.set_ylabel(y)
        ax.set_title("Bar Chart (sum by category)")
    ...
    def multi_series_plot(self):
        ...
        num_df = self.df.apply(lambda c: pd.to_numeric(c, errors="coerce"))
        keep = [c for c in num_df.columns if num_df[c].notna().sum() > 0]

        if not keep:
            return messagebox.showerror("Error", "No numeric columns found for multi-series plot.")
        ...
        ax = fig.add_subplot(111)
        for col in shown:
            s = num_df[col].dropna().reset_index(drop=True)
            if len(s) > 0:
                ax.plot(s.values, label=str(col))

        ax.legend()
        title = "Multi-Series Plot (numeric columns)"
        if len(keep) > max_cols:
            title += f" - showing first {max_cols} of {len(keep)}"
        ax.set_title(title)
        ax.set_xlabel("Index")
